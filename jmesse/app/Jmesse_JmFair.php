...
class Jmesse_JmFairManager extends Ethna_AppManager
{
	...
	*/
	function getJsonJeccMihonichiEN() {
		...
		// SQL作成
		$sql = "select jf.mihon_no mihon_no, jf.abbrev_title abbrev_title, jf.fair_title_en name, date_format(concat(jf.date_from_yyyy, '/', jf.date_from_mm, '/', jf.date_from_dd), '%b. %e, %Y') start, date_format(concat(jf.date_to_yyyy, '/', jf.date_to_mm, '/', jf.date_to_dd), '%b. %e, %Y') end, jcm_2.discription_en country, case when jf.city = '' then concat('Others(', jf.other_city_en, ')') else jcm_3.discription_en end city from jm_fair jf left outer join (select kbn_2, kbn_3, discription_jp, discription_en from jm_code_m where kbn_1 = '003' and kbn_4 = '000') jcm_2 on jf.region = jcm_2.kbn_2 and jf.country = jcm_2.kbn_3 left outer join (select kbn_2, kbn_3, kbn_4, discription_jp, discription_en from jm_code_m where kbn_1 = '003') jcm_3 on jf.region = jcm_3.kbn_2 and jf.country = jcm_3.kbn_3 and jf.city = jcm_3.kbn_4 where jf.jecc_flag in('1','2') and jf.confirm_flag = '1' and jf.del_flg = '0' and jf.web_display_type = '1' and jf.select_language_info in ('1', '2') and concat(jf.date_to_yyyy, '/', jf.date_to_mm, '/', jf.date_to_dd) >= curdate() order by jf.jecc_date desc, concat(jf.date_to_yyyy, jf.date_to_mm, jf.date_to_dd) desc ";
		$this->backend->getLogger()->log(LOG_DEBUG, '■SQL : '.$sql);
	...
	function _makeWhereForIH(&$data) {
		...
		if ('' != $search_cond['ind']) {
			if ('001' == $search_cond['ind']) {
				...
				$wherin_target1 = $this->config->get('industry_005_007_005');
				$wherin_target2 = $this->config->get('industry_005_007_006');
				$wherin_target3 = $this->config->get('industry_005_008_014');
				$wherin_target4 = $this->config->get('industry_005_009_006');
				$wherin_target5 = $this->config->get('industry_005_009_009');
				$wherin_target6 = $this->config->get('industry_005_010_001');
				$wherin_target7 = $this->config->get('industry_005_010_002');
				$wherin_target8 = $this->config->get('industry_005_010_005');
				$wherin_target9 = $this->config->get('industry_005_010_008');
				$sql_tmp_ind .= ' concat(main_industory_1, sub_industory_1) in (?,?,?,?,?,?,?,?,?) or concat(main_industory_2, sub_industory_2) in (?,?,?,?,?,?,?,?,?) or concat(main_industory_3, sub_industory_3) in (?,?,?,?,?,?,?,?,?) or concat(main_industory_4, sub_industory_4) in (?,?,?,?,?,?,?,?,?) or concat(main_industory_5, sub_industory_5) in (?,?,?,?,?,?,?,?,?) or concat(main_industory_6, sub_industory_6) in (?,?,?,?,?,?,?,?,?) ';
				array_push($data, $wherin_target1);
				array_push($data, $wherin_target2);
				array_push($data, $wherin_target3);
