...
<div id="breakdown" class="right">
	...
	<p>
		...
		<input type="checkbox" name="check_country[]" id="check_country[]" value="{$app.country_cnt[it].kbn_3}"
		...
		{$app.country_cnt[it].discription_jp} ({$app.country_cnt[it].fair_cnt})
			{else}
		<a href="{$config.url}?action_enFairList=true&type=v2&v_2={$form.v_2}&v_3={$app.country_cnt[it].kbn_3}" />{$app.country_cnt[it].discription_jp} ({$app.country_cnt[it].fair_cnt})</a>
			{/if}
		<br />
		{/section}
	</p>
	...
	<p class="narrow_btn">
		<a href="javascript:search('form_fairMenuRegion');"><img width="93" height="34" alt="Refine" src="/j-messe/images/db/btn-narrow.gif" class="over"></a>
